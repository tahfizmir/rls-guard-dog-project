...
type ProgressRow = { id: string; student_id: string; progress_json: any; updated_at: string; classroom_id?: string }
...
export default function TeacherPage() {
  ...
  async function signIn(e: React.FormEvent) {
    ...
    else setMsg('Signed in') , setSession(data.session)
  }

  async function signOut() {
    await supabase.auth.signOut()
    setSession(null); setRows([]); setMsg('Signed out')
  }
  ...
  return (
    <div>
      ...
      {!session ? (
        ...
        <div>
          <div className="mb-3">Signed in as <b>{session.user?.email ?? session.user?.id}</b></div>
          <button onClick={signOut} className="px-2 py-1 border rounded mb-4">Sign out</button>

          <h3 className="text-lg font-medium mb-2">Classroom progress (teacher)</h3>
          {loading && <div>Loading...</div>}
          {!loading && rows.length === 0 && <div>No rows found (or you may not be the teacher of any class).</div>}
          <ul className="space-y-3">
            {rows.map(r => (
              <li key={r.id} className="p-3 border rounded">
                <div className="text-xs text-gray-500">Student: {r.student_id} • Updated: {new Date(r.updated_at).toLocaleString()}</div>
